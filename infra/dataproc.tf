...
# Create the main GCS bucket for data and Dataproc temporary files.
...
resource "google_dataproc_cluster" "dataproc_cluster" {
  ...
  cluster_config {
    ...
    temp_bucket = google_storage_bucket.staging_bucket.name
    ...
    # Configure the master node (single-node cluster means only master).
    master_config {
      num_instances = 1 # Single-node cluster
      machine_type  = "n1-standard-4" # Example machine type, adjust as needed.
      disk_config {
        boot_disk_type    = "pd-standard"
        boot_disk_size_gb = 40 # Adjust disk size based on your job's needs.
      }
    }
    ...
    software_config {
      ...
      # You can add custom Spark properties here if needed for the cluster.
      # properties = {
      #   "spark:spark.history.fs.logDirectory" = "gs://${google_storage_bucket.staging_bucket.name}/spark-history"
      # }
    }
